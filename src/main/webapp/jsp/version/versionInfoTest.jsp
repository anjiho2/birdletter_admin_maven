...
function init() {
	serverKindsSelectBox("l_server_kinds", "test");
	versionService.birdLetterVersionInfo(device, "test", function(info) {
		if (info != null) {
			var checked = "";
...
function update(type) {
	...
	if (serverOnoff == "0") {
		if (confirm("점검으로 바꾸면 모든 서비스를 이용할 수 없게 됩니다.\n점검으로 바꾸시겠습니까?")) {
			versionService.updateBirdLetterVersionInfo(idx, versionNumber, serverOnoff, serverStatusCode, serverStatusContent, serverStatusTitle, "test", function(result) {
				if (result < 1) {
					alert("수정 실패했습니다.");
				} else {
					alert("수정됬습니다.");	
