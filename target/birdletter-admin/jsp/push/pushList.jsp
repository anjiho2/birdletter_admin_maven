...
function fn_search(val) {
	...
	pushService.pushInfoListCnt(statusSort, pushName, function(cnt) {
		if (cnt == "0") {
		...
		pushService.pushInfoList(sPage, statusSort, pushName, function(selList) {
			if (selList.length > 0) {
				for (var i = 0; i < selList.length; i++) {
					cmpList = selList[i];
					if (cmpList != undefined) {
						var checkHTML = "<input type='checkbox' name='chk' id='chk' value='"+cmpList.idx+"'/>";
						var nameHTML = '<a href="#" onclick="javascript:pushModify('+cmpList.idx+')">'+cmpList.name+"</a>";
		...
		<div class="subbody">
			<div class="row-fluid">
				<div class="breadcrumb span10">
					<span class="">알림</span>&nbsp;〉&nbsp;<span onclick="javascript:goPush('pushList');">기념일 알림 목록</span> 
				</div>
			<div class="span10">
					...
					<div class="content">
						<div class="portlet">
							<div class="portlet-header anthracite-gradient">
								<span class="icon_id"></span><span>기념일 알림 리스트</span>
							</div>
							...
							<p class="f_R">
								<font color="red">* 기념일 알림은 운영자가 셋팅할 수 없습니다.</font>
							</p><br><br>
							<p class="f_R">
								<font color="red">* 기념일 알림은 기념일 푸시ON 사용자 에게만 발송 됩니다.</font>
							</p><br><br>
						</div>
					</div>
				</div>
